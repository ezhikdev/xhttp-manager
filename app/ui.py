...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box}*{margin:0}html{font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color-scheme:dark;scroll-behavior:smooth}
    ...
    button,input,select{font:inherit}button{touch-action:manipulation}.topbar{position:sticky;top:0;z-index:20;height:58px;border-bottom:1px solid var(--border);background:rgba(10,10,10,.78);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px)}
...
<body>
          ...
          <form id="originForm">
            <div class="form-grid">
              ...
              <div class="field field-wide">
                ...
                <div id="stubPath" class="stub-path is-disabled"><label for="stubRoot">Каталог сайта-заглушки</label><input id="stubRoot" name="stub_root" type="text" placeholder="/var/www/stub" autocomplete="off" disabled><span class="hint">Абсолютный Linux-путь, доступный пользователю nginx</span></div><span id="sharedStubHint" class="shared-hint" hidden>Для этого домена заглушка общая. Сохранение обновит её у всех XHTTP-путей.</span>
              </div>
            </div>
  ...
  <script>
    ...
    const form=el('originForm'), list=el('originList'), alertBox=el('alert'), domainInput=el('domain'), stubEnabled=el('stubEnabled'), stubRoot=el('stubRoot'), stubPath=el('stubPath'), sharedStubHint=el('sharedStubHint'), sortSelect=el('sortMode');
    ...
    const createdValue=item=>Date.parse(item.created_at||'')||0;
    function sortedOrigins(){
      const items=[...origins];
      const tie=(a,b)=>collator.compare(a.domain,b.domain)||collator.compare(a.path,b.path)||collator.compare(a.id,b.id);
      if(sortSelect.value==='domain')return items.sort((a,b)=>collator.compare(a.domain,b.domain)||collator.compare(a.path,b.path));
      if(sortSelect.value==='path')return items.sort((a,b)=>collator.compare(a.path,b.path)||collator.compare(a.domain,b.domain));
      if(sortSelect.value==='created_asc')return items.sort((a,b)=>createdValue(a)-createdValue(b)||tie(a,b));
      return items.sort((a,b)=>createdValue(b)-createdValue(a)||tie(a,b));
    }
    function createdLabel(value){
      if(!value||value.startsWith('2000-'))return 'добавлено ранее';
      const date=new Date(value);
      if(Number.isNaN(date.getTime()))return 'время не указано';
      return 'добавлено '+new Intl.DateTimeFormat('ru-RU',{dateStyle:'short',timeStyle:'short'}).format(date);
    }
    function syncSharedStub(){
      const domain=domainInput.value.trim().toLowerCase();
      const match=origins.find(item=>item.domain===domain&&item.id!==editing);
      sharedStubHint.hidden=!match;
      if(!match)return;
      stubEnabled.checked=Boolean(match.stub_enabled);
      stubRoot.value=match.stub_root||'';
      toggleStub();
    }
    ...
    function render(){
      ...
      list.innerHTML=sortedOrigins().map(item=>`<article class="origin-item"><div class="origin-domain"><div class="domain-line"><span class="live-dot"></span>${esc(item.domain)}</div><div class="route-meta">origin · HTTP/80 · ${esc(createdLabel(item.created_at))}</div></div><div><div class="route"><code>${esc(item.path)}</code><span class="arrow">→</span><code>127.0.0.1:${item.port}</code></div><span class="tag">${item.stub_enabled?'заглушка: '+esc(item.stub_root):'остальные запросы: 404'}</span></div><div class="row-actions"><button class="btn ghost small" type="button" onclick="editOrigin('${item.id}')">Изменить</button><button class="btn danger small" type="button" onclick="deleteOrigin('${item.id}')">Удалить</button></div></article>`).join('');
    }
    ...
    function resetForm(){editing=null;form.reset();el('path').value='/xhttp';el('port').value='10086';el('formTitle').textContent='Новый origin';el('editBadge').hidden=true;el('cancelButton').hidden=true;el('saveLabel').textContent='Создать origin';sharedStubHint.hidden=true;toggleStub();showError()}
    function editOrigin(id){const item=origins.find(origin=>origin.id===id);if(!item)return;editing=id;el('domain').value=item.domain;el('path').value=item.path;el('port').value=item.port;stubEnabled.checked=item.stub_enabled;stubRoot.value=item.stub_root||'';el('formTitle').textContent='Изменить origin';el('editBadge').hidden=false;el('cancelButton').hidden=false;el('saveLabel').textContent='Сохранить изменения';toggleStub();syncSharedStub();showError();form.scrollIntoView({behavior:'smooth',block:'center'});el('domain').focus()}
    ...
    async function deleteOrigin(id){const item=origins.find(origin=>origin.id===id);if(!item||!confirm(`Удалить маршрут ${item.domain}${item.path}?`))return;showError();try{await api('origins/'+id,{method:'DELETE'});toast('Origin удалён');if(editing===id)resetForm();await load()}catch(error){showError(error.message)}}
    ...
    stubEnabled.addEventListener('change',toggleStub);domainInput.addEventListener('input',syncSharedStub);sortSelect.addEventListener('change',render);el('cancelButton').addEventListener('click',resetForm);el('rollbackButton').addEventListener('click',rollback);toggleStub();load();
  </script>
</body>
</html>'''
