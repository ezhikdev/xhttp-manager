...
def render_domain(domain, routes):
    ...
    stub_source = routes[0]
    stub = f"root {stub_source['stub_root']};\n        try_files $uri $uri/ =404;" if stub_source["stub_enabled"] else "return 404;"
    route_ids = ", ".join(item["id"] for item in routes)
    locations = "\n\n".join(render_route(item) for item in routes)
    return f'''# Generated by XHTTP Manager: {route_ids}
server {{
    listen 80;
    listen [::]:80;
    server_name {domain};
