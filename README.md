...
- несколько XHTTP-путей на одном домене с общей синхронизируемой заглушкой;
- сортировка маршрутов по домену, пути и времени добавления;
- настройка отдельного XHTTP-префикса и локального порта Xray, включая дочерние URI сессий;
- проксирование только на `127.0.0.1`;
- отключённые буферизация, gzip и кэширование для XHTTP;
- общий keepalive-пул на каждый уникальный локальный порт Xray и очистка заголовка `Connection`;
- заголовки `X-Forwarded-Proto`, `Cache-Control`, `Pragma` и `X-Accel-Buffering`;
- выдача сайта-заглушки или фирменной страницы 404 для остальных запросов;
- один nginx `server` на домен с отдельными XHTTP-location для каждого пути;
